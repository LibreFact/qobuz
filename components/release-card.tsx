...
interface Track {
  ...
  track_number: number
  media_number: number
  previewUrl?: string // Added for preview URL
}
...
export function TrackList({ tracks, artistName, albumName }: TrackListProps) {
  return (
    <div className="grid grid-cols-1 gap-1">
      {tracks.map((track) => (
        <div key={track.id} className="group flex items-center justify-between py-2 px-3 rounded-md hover:bg-muted/50">
          <div className="flex items-center gap-3">
            <div className="text-muted-foreground w-6 text-center">{track.track_number}</div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <div className="text-muted-foreground text-sm">{formatDuration(track.duration)}</div>
          </div>
        </div>
      ))}
    </div>
